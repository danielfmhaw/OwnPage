...
func GetWareHouseParts(w http.ResponseWriter, _ *http.Request) {
	utils.HandleGet(w, `
		SELECT 
			...
			CASE 
				...
				ELSE NULL
			END AS part_name,
			wp.quantity, wp.storage_location
	...
	`, func(scanner utils.Scanner) (any, error) {
		...
		err := scanner.Scan(&p.ID, &p.PartType, &p.PartID, &p.PartName, &p.Quantity, &p.StorageLocation)
		return p, err
	})
}
...
func DeleteWarehousePart(w http.ResponseWriter, r *http.Request) {
	...
	utils.HandleDelete(w, "DELETE FROM warehouse_parts WHERE id = $1", id)
}
...
func UpdateWarehousePart(w http.ResponseWriter, r *http.Request) {
	...
	err = utils.HandleUpdate(w, query, part.PartType, part.PartID, part.Quantity, part.StorageLocation, part.ID)
}
...
func InsertWarehousePart(w http.ResponseWriter, r *http.Request) {
	...
	query := `INSERT INTO warehouse_parts (part_type, part_id, quantity, storage_location) VALUES ($1, $2, $3, $4)`
	err = utils.HandleInsert(w, query, part.PartType, part.PartID, part.Quantity, part.StorageLocation)
}
